export const projects = [
  {
    ...
    본격적으로 웹 개발자가 되기로 결정하기 전에 진행한 프로젝트입니다. 웹 개발과는 무관하지만 복잡했던 회로 조립, 학교에서 배우지 않았던 납땜질과 아두이노 시스템 프로그래밍은 개인적으로 값진 경험이었습니다.<br><br>
    <strong>배경</strong><br>
  ...
  },
  {
    ...
    description: `<strong>개요</strong><br>
    Music Diary는 사용자의 감정과 선호도에 맞는 음악을 추천하는 웹 애플리케이션입니다. Next.js를 기반으로 하여, 사용자가 작성한 일기를 분석하고 그에 맞는 음악을 추천하는 기능 및 감정 히스토리를 정리해주는 기능을 제공합니다.<br><br>
    <strong>배경</strong><br>
    ...
    또한 비동기 작업 처리를 위해 Next.js의 새로운 기능인 Server Actions를 사용해보았으며, 최신 데이터를 자동적으로 받아 오는 React Query를 통해 클라이언트의 다이나믹한 상태를 훨씬 쉽게 작업할 수 있다는 것을 알게 되었습니다.<br>
    Next.js의 탁월한 성능 최적화를 최대한 느껴보고 싶었지만, 아무래도 데이터의 실시간 업데이트가 많이 필요하지 않은 소규모 애플리케이션에서는 성능상 큰 체감을 느끼지는 못하였습니다. 하지만 대부분의 정보들이 HTML DOM에 포함된 상태로 클라이언트로 전달되는 것을 확인하고 나서는, 확실히 SEO 최적화를 위해서는 Next.js가 적절한 선택이라는 생각이 들었습니다.<br><br>
    <strong>고민의 흔적</strong><br>
    사용자가 작성한 일기 내용으로부터 감정을 분석하기 위해서 Naver CLOVA API를 사용했지만 해당 API는 감정을 (긍정, 부정, 중립) 이렇게 세 단계로만 분석해주는 한계가 있었습니다. 그래서 일기 내용에서 가장 많이 등장하는 단어들에게 가중치 부여한 후 나열해서, 관련된 내용의 가사를 가지는 음악을 추천하는 방식을 시도해보았으나, 단순히 등장하는 단어의 개수로 음악을 추천받기에는 뜬금 없는 음악을 추천해주고 있었습니다. 비용이 들긴 하지만 어쩔 수 없이 일기 내용과 관련된 음악을 추천받기에는 OpenAI의 API를 사용하는 것이 가장 적합할 것 같다는 생각이 들어서 OpenAI API를 사용하여 음악 추천 및 약 일주일 간의 일기 내용을 장문으로 정리해주는 기능을 추가하였습니다. 첨언하자면, 일주일 간의 일기 내용은 생각보다 많은 문자를 포함하고 있어서, 한 번의 API 호출로 0.15달러 이상이 소비되는 것을 보고, 이 어플리케이션은 상업적인 한계점을 가지고 있다고 생각하였습니다.<br>또한 Tailwind CSS 유틸리티 클래스를 활용하여 반응형 디자인을 구현한 상태라서, <a href="https://github.com/backbone94/music-recommendation-mobile" target="_blank" class="text-blue-500">React Native의 WebView</a>를 사용하면 웹 기반의 앱을 스마트폰의 앱으로 배포할 수 있는 것을 알게 되어 실제로 앱 배포를 하려고 했으나, 구글의 플레이스토어에서 개발자 계정을 심사받는 과정이 생각보다 까다롭다는 사실을 알게되었습니다. 계정 심사를 모두 받은 이후에는 다시 한 번 앱 배포를 시도해보고자 합니다.`,
  },
  {
    ...
    UI 라이브러리는, 구글의 Material Design 가이드라인을 바탕으로 만들어진 MUI(Material UI) 라이브러리를 활용하여 React의 컴포넌트와 잘 어우러지도록 UI를 개선하였습니다.<br><br>
    <strong>고민의 흔적</strong><br>
    Figma 플러그인 위에 지도를 표시하기 위해, 초반에는 Leaflet 라이브러리를 사용하여 개발 진행을 하고 있었으나, Leaflet 라이브러리는, 지도 위에 Polygon을 그리는 기능과, Polygon의 모양 변형, 드래그, 리사이즈 등의 기능을 구현하기 위한 커스텀을 제공하지 않는 고도화된 지도 라이브러리가 아니었기에, 사수분과의 상담를 통해 Leaflet 라이브러리보다 더 다양한 커스텀 기능을 제공하는 OpenLayers 라이브러리로 대체하는 방식으로 수정하여 지도 구현을 하였습니다.<br>
    또한 Polygon을 지도의 특정 위치에 배치시켜야 할 때, 지도를 직접 드래그 하면서 그 지역을 찾으려고 하는 게 매우 비효율적이다라는 생각을 하게 되었고, 인턴 프로젝트로써 예정에 없었던 지도 검색 기능을 Nominatim 라이브러리를 사용하여 추가하였습니다.<br>
    이 플러그인의 핵심 기능인, Polygon의 위경도 좌표 변환을 위해서는 좌표계에 대한 개념을 알아야 했는데, OpenLayers 라이브러리가 사용하고 있는 EPSG:3857 좌표계와, 위경도 좌표를 나타내는 데에 사용되는 EPSG:4326 좌표계와 같은 생소한 개념들을 알지 못한 채로 작업을 진행해서 중간에 좌표 변환이 제대로 이뤄지지 않는 바람에 조금 힘들었던 경험을 하였습니다.<br>
  ...
  },
  {
    ...
    description: `<strong>개요</strong><br>
    아주대학교 소프트웨어학과 졸업 작품입니다. 기본적으로는 일회성 모임을 위한 어플리케이션이며, 자신이 참여한 모임을 기념하기 위한 "증표"를 생성하여 간직하거나, 개인적으로 증표를 커스텀하여 친구들과 공유할 수 있도록 해주는 기능을 제공합니다.<br><br>
    <strong>배경</strong><br>
    ...
    또한 클라이언트에서의 백엔드 API 호출 시 클라이언트 인증을 위해, 서버에 세션 저장소를 따로 둘 필요가 없는 JWT 방식을 도입하였습니다. JWT 방식의 보안 허점을 조금이라도 더 보완하고자 AWS의 인메모리 캐싱 서비스인 ElastiCache(Redis)에, JWT가 만료됐을 때를 대비한 Refresh Token을 따로 저장하는 방식을 사용하여 보안성을 향상시켰습니다.<br><br>
    <strong>고민의 흔적</strong><br>
    Spring에 익숙하지 않은 상태에서 프로젝트를 진행하는 것은 도전적인 경험이었지만, GitHub Actions를 통한 CI/CD의 구축과, AWS의 EC2 인스턴스, RDS, ElastiCache와 같은 클라우드 환경의 운영을 직접 경험할 수 있었던 소중한 시간들이었습니다.<br>
    GitHub Actions를 구축하면서, Mock 라이브러리로 작성된 테스트 코드를 통해 배포 전 코드의 안정성을 높이기 위한 CI를 프로젝트 초반에는 계속 시도하였으나, 생각보다 테스트 코드 작성에 꽤 많은 시간이 드는 바람에 프로젝트의 진척도가 더뎠고, 그로 인해 교수님의 압박이 있었습니다. 따라서 아쉽지만 테스트 코드는 작성하지 않는 방향으로 진행을 했었는데, 테스트 코드 작성을 하지 못한 부분은 꽤 아쉬운 부분으로 남게 되었습니다.<br>
    프론트엔드 팀원과의 협업 과정에서, 백엔드 측에서 API를 만들었다고 해도 프론트엔드 팀원이 해당 API에 대한 정보가 왜곡된 채로 적절한 API 사용을 하지 못하는 문제가 있었습니다. 이러한 문제를 해결하고, 프론트엔드 팀원과의 원활한 소통을 위해서는 API 문서를 작성하는 것이 좋을 것 같다고 생각하여 백엔드 팀원들과 협의 후 Swagger를 사용하여 API 문서를 만들어서 프론트엔드 팀원들이 작업을 좀더 수월하게 진행할 수 있도록 했습니다. 열심히 만든 API를 문서로써 보기 좋게 정리하여 프론트엔드 팀원들이 API 문서를 효율적으로 사용하는 모습이 뿌듯하였습니다.<br>
    실시간 채팅 기능 개발은 WebSocket 프로토콜을 활용하였는데, 익숙했던 HTTP 통신과 달리 디버깅 과정에서 오류 메시지가 구체적으로 나오지 않는 바람에 디버깅에 생각보다 애를 먹었습니다.<br>
    그리고 기획 단계에서 언급만 됐었던, Open AI API를 통한 이미지 생성 기능을, 촉박한 시간으로 인해 생략하려고 했었지만 최신 AI 기술을 꼭 적용해보고 싶었던 개인적인 욕심에, 프로젝트 마지막쯤에 OpenAI Dall-E 3 API를 사용하여, 모임의 성격을 프롬프트로 요청하면 모임과 관련된 증표 이미지를 생성해주는 기능을 추가하였습니다.<br>
    이전에 프론트엔드(Angular) 개발자로써의 경력을 가진 상태로 프로젝트를 진행했다 보니, 프론트엔드 팀원이 속도가 나지 않는 상황일 때 제가 프론트엔드, 백엔드 개발을 동시에 진행하면서 시간적으로 부족한 부분들을 최대한 메꾸려는 시도를 하였고, 그로 인해 프로젝트에 좀더 많은 기여를 하게 되었습니다.`,
  },
  {
    ...
    description: `<strong>개요</strong><br>
    저의 첫 번째 포트폴리오 사이트 프로젝트입니다.<br><br>
    <strong>배경</strong><br>
    해당 사이트는 간단하고 심플한 포트폴리오 사이트를 만드는 것을 목표로 했기 때문에 기본적인 웹 기술인 HTML, CSS, JavaScript를 사용하여 개발되었습니다.<br><br>
    <strong>고민의 흔적</strong><br>
  ...
  },
  {
    ...
    description: `<strong>개요</strong><br>
    첫 번째 포트폴리오 사이트를 새롭게 개편한 리뉴얼 포트폴리오 프로젝트입니다.<br><br>
    <strong>배경</strong><br>
    ...
    컴포넌트의 스타일 커스터마이징을 손쉽게 할 수 있는 Tailwind CSS를 통해서 첫 번째 포트폴리오 프로젝트에서 문제가 되었던 반응형 디자인을 해결하여, 모바일 환경에서도 디자인이 깨지지 않는 일관된 디자인으로 개발하였습니다.<br><br>
    <strong>고민의 흔적</strong><br>
  ...
  },
  {
    ...
    description: `<strong>개요</strong><br>
    Blog는 저의 첫 CRUD 웹사이트 프로젝트입니다. 개발자라면 자신만의 기록 공간이 필수적으로 있어야 한다고 생각했기 때문에 첫 번째 프로젝트로 블로그를 만들어 보았습니다.<br><br>
    <strong>배경</strong><br>
    이 프로젝트를 통해 첫 React SPA 사이트를 구성하였고, 서버와의 비동기 통신을 수행하기 위해 Redux Saga를 활용하였습니다. 또한 텍스트 에디터 중 하나인 CKEditor를 활용하여 글쓰기 기능을 구현하였습니다. 프로필 이미지는 AWS의 S3에 업로드할 수 있게 개발하였고 AWS EC2 인스턴스를 활용하여 실제 배포 단계까지 밟아보았습니다.<br><br>
    <strong>고민의 흔적</strong><br>
    Redux Saga를 처음 학습할 때에도 러닝커브에 대한 각오를 하고 학습을 시작하였지만, 개발하는 과정 속에서 반복되고 장황한 코드 패턴과, 생소한 generator 함수에 좌절했던 기억이 있습니다. 지금 생각해보면, 소규모로 진행했던 프로젝트였던 것만큼 Redux Saga를 썼다는 사실이 비효율적인 방식이었다는 생각이 들지만, 이 프로젝트를 통해서 비동기 통신에 대한 개념을 정립할 수 있었습니다.<br>
  ...
  },
  {
    ...
    description: `<strong>개요</strong><br>
    Blog 만든 경험을 바탕으로, 제가 좋아하는 영화에 대한 프로젝트를 만들어 보았습니다. 좋아하는 분야에 대한 프로젝트를 진행해서인지 Blog 프로젝트를 진행했을 때보다 훨씬 더 흥미롭게 진행했습니다.<br><br>
    <strong>배경</strong><br>
    ...
    <strong>고민의 흔적</strong><br>
    이 프로젝트에서는 Next.js를 왜 쓰는지 모른 채로 Next.js를 사용하여 진행한 첫 프로젝트였는데, 폴더 구조를 통한 편리한 페이지 라우팅 기능을 경험할 수 있었습니다.(라우팅 기능 말고는 Next.js의 여러 기능들은 이 프로젝트에 적용하지 않았습니다.)<br>
  ...
  },
  {
    ...
    description: `
    이전 회사에서 진행했었던 프로젝트들을 모아봤습니다. 이전 회사에서는 PDF를 웹 상에서 실시간으로 수정할 수 있는 솔루션에 대한 프론트엔드 기능 개발 및 유지보수 업무를 맡았습니다. 버그 수정 또는 이슈 해결과 같이 자잘하게 진행했었던 작업들은 제외하고, 제가 핵심적으로 참여했던 프로젝트로는 우클릭 커스텀 기능, 비가시 추적코드 추가, 다크모드 기능이 있습니다.<br><br>
