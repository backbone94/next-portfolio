...
export const projects: Project[] = [
  {
    ...
    프론트엔드만 하던 시절에는 "배포는 되는 것"이었는데, 직접 다루면서 서비스가 끊기지 않게 바꾸는 일이 따로 있다는 걸 알게 됐습니다.<h3>검색 결과에 병원을 정확히 알리기</h3>
    검색엔진은 페이지를 읽어도 어디가 병원 이름이고 어디가 주소인지 스스로 알지 못합니다. 병원 정보 · 진료 페이지 탐색경로 · 질환 정보 콘텐츠에 <strong>구조화 데이터(JSON-LD)</strong>를 붙여 이걸 명시했습니다.<br><br>
    질환 정보 페이지에는 저자 표시가 필요했는데, 개별 의료진이 건별로 검수했다는 근거가 없는 상태에서 "OOO 원장 감수"처럼 특정인을 지목하면 허위 표시가 됩니다. 대신 <strong>병원이라는 기관 단위</strong>로 저작 주체를 명시해 사실과 다른 신호를 보내지 않도록 했습니다.<br><br>
    관리자가 이 정보를 직접 다루는 SEO 설정 화면도 손을 봤습니다. 화면 두 개에 걸쳐 있던 SEO 관련 설정을 한 곳으로 모으고, 실제로는 적용되지 않던 입력칸(코드가 참조하지 않거나 보안 위험이 있어 백엔드가 의도적으로 무시하던 값)을 걷어냈습니다. 정리 도중 <strong>일본어·중국어 SEO 설정이 실제 사이트가 쓰는 언어 코드와 달라</strong> 등록해도 반영되지 않던 버그도 함께 잡았습니다.`,
  },
  {
