export const projects = [
  ...
  },
  {
    ...
    description: `아주대학교 소프트웨어학과 졸업 작품입니다. 기본적으로는 일회성 모임을 위한 어플리케이션이며, 자신이 참여한 모임을 기념하기 위한 "증표"를 생성하여 간직하거나 개인적으로 증표를 커스텀하여 친구들과 공유할 수 있도록 해주는 기능을 제공합니다. 저는 프론트엔드 개발자를 목표로 삼고 있었지만, 웹 개발자로서의 시야를 넓히기 위해 이 프로젝트에서는 백엔드 개발을 맡았습니다.<br><br>Spring에 익숙하지 않은 상태에서 프로젝트를 진행하는 것은 도전적인 경험이었지만, CI/CD의 구축과 AWS EC2 인스턴스의 운영에 대해서 학습할 수 있었으며, 프론트엔드와의 협업 과정에서 API 설계의 중요성을 알 수 있는 계기가 되었습니다. 프로젝트 진행 중간에 API에 대한 정리가 필요할 것으로 판단되어 Swagger API 문서를 만들어서 프론트엔드 팀원들이 작업을 좀더 수월하게 할 수 있도록 진행했습니다. 또한 JWT(+Redis ElastiCache)와 OAuth 2.0을 활용한 (카카오, 구글, 네이버) 소셜 로그인 기능 구현을 통해 보안과 인증 처리에 대한 경험을 쌓았으며, WebSocket을 이용한 실시간 채팅 기능 개발을 통해 웹 애플리케이션의 실시간 처리 능력을 향상시켰습니다. 그리고 기획 단계에서 언급만 됐었던, Open AI API를 통한 이미지 생성 기능을, 촉박한 시간으로 인해 생략하려고 했었지만 최신 AI 기술을 꼭 적용해보고 싶었던 개인적인 욕심에, 프로젝트 마지막쯤에 OpenAI Dall-E 3 API를 사용하여, 모임의 성격을 프롬프트로 요청하면 관련된 증표 이미지를 생성해주는 기능을 추가하였습니다. <br><br>이전에 프론트엔드(Angular) 개발자로써의 경력을 가진 상태로 프로젝트를 진행했다 보니, 프론트엔드 팀원이 속도가 나지 않는 상황일 때 제가 프론트엔드, 백엔드 개발을 동시에 진행하면서 시간적으로 부족한 부분들을 최대한 메꾸려는 시도를 하였고, 그로 인해 프로젝트에 좀더 많은 기여를 하게 되었습니다.`,
  },
  {
  ...
  },
  {
    ...
    description: `저의 첫 번째 포트폴리오 사이트 프로젝트입니다. 해당 사이트는 HTML, CSS, JavaScript로 개발되었으며, 기본적인 웹 개발 기술을 사용하여 간단하고 효과적인 포트폴리오를 만드는 것을 목표로 했습니다. 그러나 반응형 디자인이 충분히 구현되지 않아 모바일 환경에서 디자인이 깨지는 문제가 있었습니다. 이 과정에서 프레임워크 없이 개발하는 것의 한계를 느끼며, 더 나은 반응형 디자인과 유지보수성을 위해 리뉴얼을 결심하게 되었습니다.`,
  },

  {
    ...
    description: `첫 번째 포트폴리오 사이트를 새롭게 개편한 리뉴얼 포트폴리오 프로젝트입니다. 리뉴얼된 포트폴리오 사이트는 Next.js와 Tailwind CSS를 사용하여 개발되었으며, 이번 리뉴얼에서는 SEO 최적화를 적용하고, 반응형 디자인을 개선하여 모바일 환경에서도 디자인이 깨지지 않도록 구현했습니다. Tailwind CSS의 유틸리티 클래스와 Next.js의 정적 사이트 생성(SSG) 및 서버 사이드 렌더링(SSR)을 활용하여 디자인 일관성과 유지보수성을 높였습니다. 리뉴얼을 통해 반응형 웹 디자인의 중요성을 이해하게 되었고, 사용자 경험을 고려한 웹 개발을 더욱 잘 구현할 수 있게 되었습니다.<br><br>이 포트폴리오 페이지는 새로운 정보와 프로젝트들로 계속해서 채워질 예정입니다.`,
  },
];
