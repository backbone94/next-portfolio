export const projects = [
  ...
  },
  {
    ...
    또한 Tailwind CSS 유틸리티 클래스를 활용하여 반응형 디자인을 구현한 상태라서, <a href="https://github.com/backbone94/music-recommendation-mobile" target="_blank" class="text-blue-500">React Native의 WebView</a>를 사용하면 웹 기반의 앱을 스마트폰의 앱으로 배포할 수 있는 것을 알게 되어 실제로 앱 배포를 하려고 했으나, 구글의 플레이스토어에서 개발자 계정을 심사받는 과정이 생각보다 까다롭다는 사실을 알게되었습니다. 계정 심사를 모두 받은 이후에는 다시 한 번 앱 배포를 시도해보고자 합니다.
    `,
  },
  {
    title: '살롱 | SSALON',
    ...
    Spring에 익숙하지 않은 상태에서 프로젝트를 진행하는 것은 도전적인 경험이었지만, GitHub Actions를 통한 CI/CD의 구축과, AWS의 EC2 인스턴스, RDS, ElastiCache와 같은 클라우드 환경의 운영을 직접 경험할 수 있었던 소중한 시간들이었습니다.<br><br>
    GitHub Actions를 구축하면서, Mock 라이브러리로 작성된 테스트 코드를 통해 배포 전 코드의 안정성을 높이기 위한 CI를 프로젝트 초반에는 계속 시도하였으나, 생각보다 테스트 코드 작성에 꽤 많은 시간이 드는 바람에 프로젝트의 진척도가 더뎠고, 그로 인해 교수님의 압박이 있었습니다. 따라서 아쉽지만 테스트 코드는 작성하지 않는 방향으로 진행을 했었는데, 테스트 코드 작성을 하지 못한 부분은 꽤 아쉬운 부분으로 남게 되었습니다.<br><br>
    ...
    그리고 기획 단계에서 언급만 됐었던, Open AI API를 통한 이미지 생성 기능을, 촉박한 시간으로 인해 생략하려고 했었지만 최신 AI 기술을 꼭 적용해보고 싶었던 개인적인 욕심에, 프로젝트 마지막쯤에 OpenAI Dall-E 3 API를 사용하여, 모임의 성격을 프롬프트로 요청하면 모임과 관련된 증표 이미지를 생성해주는 기능을 추가하였습니다.<br><br>
  ...
  },
  {
    ...
    Redux Saga를 처음 학습할 때에도 러닝커브에 대한 각오를 하고 학습을 시작하였지만, 개발하는 과정 속에서 반복되고 장황한 코드 패턴과, 생소한 generator 함수에 좌절했던 기억이 있습니다. 지금 생각해보면, 소규모로 진행했던 프로젝트였던 것만큼 Redux Saga를 쓴다는 사실이 비효율적인 방식이었다는 생각이 들지만, 이 프로젝트를 통해서 비동기 통신에 대한 개념을 정립할 수 있었습니다.<br><br>
  ...
  },
  {
    ...
    이 프로젝트에서는 Next.js의 사용 목적을 알지 못한 채로 무작정 사용해본 프로젝트였으나, Next.js의 폴더 구조를 통한 편리한 페이지 라우팅 기능만큼은 눈에 띄는 경험이었습니다.<br>
  ...
  },
  {
    ...
    인턴으로서 참여한 첫 프로젝트입니다. 프로젝트의 목표는 Figma의 특정 레이어를 지도상의 특정 위치에 배치하고, 해당 위치의 위경도 좌표를 기억할 수 있는 Figma 플러그인을 개발하는 것이었습니다.<br><br>
    ...
    또한 Polygon을 지도의 특정 위치에 배치시켜야 할 때, 지도를 직접 드래그 하면서 그 지역을 찾으려고 하는 게 매우 비효율적이다라는 생각을 하게 되었고, 지도 검색 기능을 Nominatim 라이브러리를 사용하여 추가하였습니다.<br><br>
